 ...
 * -------------------------------------------------------------
 */
juju_void bErrorPrint(juju_char* fmt, ...);
juju_void displayBuffer(BufferReader* ptr_Buffer);
juju_long getFileSize(juju_char* fname);
juju_int isNumber(const juju_char* ns);
juju_void startReader(juju_char*, juju_char*, juju_char, juju_int, juju_int);
...
juju_int mainReader(juju_int argc, juju_char** argv) {
	...
	juju_char* program = argv[0];
	juju_char* input = argv[2];
	juju_char mode = MODE_FIXED;
	juju_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
juju_void startReader(juju_char* program, juju_char* input, juju_char mode, juju_int size, juju_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	juju_int loadSize = 0;		/* the size of the file loaded in the buffer */
	juju_char symbol;			/* symbol read from input file */
...
juju_void displayBuffer(BufferReader* ptr_Buffer) {  
	...
	printf("\nNumber of different chars read: %d\n",
