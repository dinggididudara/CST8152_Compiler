...
juju_int line;								/* Current line number of the source code */
extern juju_int errorNumber;				/* Defined in platy_st.c - run-time error number */

extern juju_int stateType[];
extern juju_char* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern juju_int transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(juju_void) {
	...
	juju_char c;	/* input symbol */
	juju_int state = 0;		/* initial state of the FSM */
	juju_int lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	juju_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	juju_int lexLength;		/* token length */
	juju_int i;				/* counter */
	juju_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((juju_int)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
juju_int nextState(juju_int state, juju_char c) {
	juju_int col;
	juju_int next;
	col = nextClass(c);
...
Token funcIL(juju_char lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (juju_int)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcSL(juju_char lexeme[]) {
	Token currentToken = { 0 };
	juju_int i = 0, len = (juju_int)strlen(lexeme);
...
juju_void printToken(Token t) {
	...
	/* External variables */
	extern juju_char* keywordTable[]; /* link to keyword table in */
	...
	case MNID_T:
		...
		break;
	case STR_T:
		printf("STR_T\t\t%d\t ", (juju_int)t.attribute.codeType);
		printf("%s\n", readerGetContent(stringLiteralTable, (juju_int)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
