...
juju_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern juju_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern juju_int line; /* source code line number - defined in scanner.c */
...
extern juju_void startParser(julius_void);
extern juju_int startScanner(ReaderPointer sc_buf);

static juju_void printParserError(juju_char* fmt, ...);
static juju_void displayParser(ReaderPointer ptrBuffer);
static juju_long getParserFileSize(juju_char* fname);
static juju_void callGarbageCollector(julius_void);
...
juju_int mainParser(juju_int argc, juju_char** argv) {
	...
	juju_int loadsize = 0; /*the size of the file loaded in the buffer */
...
juju_void printParserError(juju_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(juju_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
juju_long getParserFileSize(juju_char* fname) {
	FILE* input;
	juju_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
juju_void displayParser(ReaderPointer ptrBuffer) {
...
juju_void callGarbageCollector(julius_void) {
